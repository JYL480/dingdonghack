...
from pipeline import nav_agent
from pipeline.ui_test_agent import *
    ...
    try:
        ...
        try:
            ...
            if labeled_image_path and os.path.exists(labeled_image_path):
                ...
                print(f"✓ Saved action visualization: {viz_result.get('clicked_image', 'N/A')}")
            ...
            print(f"⚠️ Could not save action visualization: {str(viz_error)}")
...
def execute_task(
    ...
    print("📝 Using direct React mode execution (no GraphDB)")
...
async def run_multiple_tasks(tasks: list, device: str = "emulator-5554") -> Dict[str, Any]:
    """
    ...
    for idx, task in enumerate(tasks, 1):
        ...
        result = await run_task_with_folder(task, device, task_subfolder)  # Awaiting task execution
        results.append({
            "task": task,
            "result": result,
        ...
        # Brief pause between tasks (non-blocking)
        if idx < len(tasks):
...
async def run_task_with_folder(task: str, device: str = "emulator-5554", folder: str = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        print("🤖 Compiled workflow, starting execution...", app)
...
async def run_task(task: str, device: str = "emulator-5554") -> Dict[str, Any]:
    """
    ...
    if ';' in task or '\n' in task:
        ...
        if len(tasks) > 1:
            print(f"📝 Detected {len(tasks)} tasks, running in sequence...")
            return await run_multiple_tasks(tasks, device)  # Awaiting the result here
    
    # Single task execution
    return await run_task_with_folder(task, device)  # Awaiting single task execution
...
async def capture_and_parse_screen(state: DeploymentState) -> DeploymentState:
    """
    ...
    try:
        ...
        screen_result = await screen_element.ainvoke({"image_path": screenshot_path})
...
async def fallback_to_react(state: DeploymentState) -> DeploymentState:
    """
    ...
    print("action_agent created successfully")
...
Each step of the operation should move toward completing the user's goal task."""
    ...
    state = await capture_and_parse_screen(state)
    if not state["current_page"]["screenshot"]:
...
Only execute one tool call."""
    ...
    print("Action agent invoked successfully", action_result)
...
async def capture_screen_node(state: DeploymentState) -> DeploymentState:
    ...
    updated_state = await capture_and_parse_screen(state_dict)  # Await the async function
...
async def match_elements_node(state: DeploymentState) -> DeploymentState:
    """
...
async def execute_action_node(state: DeploymentState) -> DeploymentState:
    """
    Execute operation
    """
    state_dict = dict(state)
    ...
    if state["should_execute_shortcut"] and state["execution_template"]:
        ...
        # Call execute_high_level_action function asynchronously
        result = await execute_high_level_action(
        ...
        # Call match_task_to_action function asynchronously
        is_matched, matched_action = await match_task_to_action(state_dict, state["task"])
...
async def fallback_node(state: DeploymentState) -> DeploymentState:
    """
    ...
    state = await fallback_to_react(state)
...
async def is_task_completed(state: DeploymentState) -> str:
    """
...
async def check_task_completion(state: DeploymentState) -> DeploymentState:
    """
    ...
    completion_response = await model.invoke(completion_messages)
    ...
    judgement_response = await model.ainvoke(all_messages)
...
async def build_workflow() -> StateGraph:
    ...
    workflow.add_node("ui_test_agent_node", ui_test_agent_node)
    ...
    # Do the UI testing in the workflow
    workflow.add_edge("check_completion", "ui_test_agent_node")

    workflow.add_edge("ui_test_agent_node",END)
