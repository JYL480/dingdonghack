...
class Neo4jDatabase:
    def __init__(self, uri: str, auth: tuple):
        """Initialize Neo4j database connection with retries for Aura"""
        self.uri = uri
        self.auth = auth
        self.driver = None
        self.connect_with_retries()

    def connect_with_retries(self, max_retries: int = 5):
        """Connect to Neo4j with retries - handles Aura cold starts"""
        for attempt in range(max_retries):
            try:
                print(f"Connecting to Neo4j Aura (attempt {attempt + 1}/{max_retries})...")
                # For neo4j+s:// (secure), no additional trust config needed
                # The +s already indicates secure connection
                self.driver = GraphDatabase.driver(
                    self.uri, 
                    auth=self.auth
                )
                
                # Use verify_connectivity as recommended by Neo4j
                ...
                error_msg = str(e)
                if "Unable to retrieve routing information" in error_msg or "ServiceUnavailable" in str(type(e)):
                    if attempt < max_retries - 1:
                        wait_time = 10 + (attempt * 5)  # Longer wait for Aura
                        print(f"  Connection failed. Waiting {wait_time} seconds before retry...")
                        print(f"  Error: {error_msg}")
                        time.sleep(wait_time)
                    else:
                        print("\n[ERROR] Failed to connect to Neo4j Aura after multiple attempts.")
                        print(f"\nLast error: {error_msg}")
                        print("\nPossible solutions:")
                        print("1. Go to https://console.neo4j.io and check if your instance is running")
                        print("2. Verify the URI in config.py matches your instance: neo4j+s://cbd4231c.databases.neo4j.io")
                        print("3. Check your username (should be 'neo4j') and password in config.py")
                        print("4. Try connecting from Neo4j Browser to verify credentials")
                        print("5. Your instance might need more time to warm up (try again in 2-3 minutes)")
                        sys.exit(1)
                else:
                    raise
                ...
                print("Please verify your Neo4j credentials in config.py")
                print(f"URI: {self.uri}")
                print(f"Username: {self.auth[0]}")
                sys.exit(1)
