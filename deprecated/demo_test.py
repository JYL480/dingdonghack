...
"""
        ...
        with gr.Tab("Action Execution"):
            ...
            def execute_deployment_task(
                device: str, task_description: str, progress=gr.Progress()
            ):
                """
                Handles the execution of a deployment task in a background thread
                while providing real-time UI updates to a Gradio interface.
                """
                if not device or device == "No devices found":
                    return (
                ...
                try:
                    ...
                    async def run_in_background(task_desc: str, dev: str):
                        """
                        The core async function that patches and runs the deployment task.
                        """
                        # --- FIX 2: Get the original function from the module object ---
                        original_run_task = deployment_test.run_task
                        try:
                            async def patched_run_task(task, device):
                                """A temporary wrapper to intercept the result for UI updates."""
                                add_log("Initializing task execution...")
                                # Call the stored original function
                                result = await original_run_task(task, device)
                                
                                # Logical Fix: Determine completion status based on the result's 'status' key
                                is_completed = result.get("status") == "success"
                            ...
                            # Apply the patch to the module's run_task attribute
                            deployment_test.run_task = patched_run_task

                            add_log("Starting task execution process...")
                            # --- FIX 3: Call the function VIA THE MODULE to use the patched version ---
                            await deployment_test.run_task(task_desc, dev)

                        except Exception as e:
                            error_message = f"Error during background execution: {str(e)}\n{traceback.format_exc()}"
                            add_log(error_message)
                            update_queue.put({"status": "error", "message": str(e), "completed": False})
                        finally:
                            # CRITICAL: Always restore the original function to avoid side-effects
                            deployment_test.run_task = original_run_task

                    def run_wrapper(task_desc: str, dev: str):
                        """Synchronous bridge to run the async function in a new event loop."""
                        asyncio.run(run_in_background(task_desc, dev))

                    # Start the background thread targeting the synchronous wrapper
                    thread = threading.Thread(target=run_wrapper, args=(task_description, device))
                    thread.start()

                    # UI update loop (generator)
                    while thread.is_alive():
                        progress((len(logs) * 0.05) % 0.9, "Executing task...")
                        yield "\n".join(logs), "Executing...", screenshots
                        time.sleep(0.5)

                    # Final processing after the thread has finished
                    thread.join()

                    try:
                        # Get the final result that was put in the queue
                        ...
                        if status == "success" and completed:
                            ...
                            final_status = "⚠️ Execution finished but task not fully completed"
                            add_log(f"⚠️ Execution finished with status: {status}")
                        else:
                            final_status = f"❌ Execution failed: {message}"
                            add_log(f"❌ Task execution failed: {message}")
                    except Exception:
                        final_status = "❓ Unable to retrieve final execution result"
                        add_log("❓ Unable to retrieve final execution result")

                    progress(1.0, "Execution finished")
                    add_log("Task execution process finished")

                    return "\n".join(logs), final_status, screenshots

                except Exception as e:
                    error_message = f"Fatal error in Gradio handler: {str(e)}\n{traceback.format_exc()}"
                    add_log(error_message)
